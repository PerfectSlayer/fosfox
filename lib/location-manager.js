...
function getByDomain(domain) {
	...
	if (Prefs.locationStrategy === 'useDefault')
		...
		return {'path': Prefs.defaultLocation, 'always': false};
	/*
 ...
 */
function getDefault() {
	// Check use default location strategy
	if (Prefs.locationStategy === 'useDefault')
		// Return default location from preference
...
/*
 * Add a path for a domain.
 */
function save(domain, path, always) {
	// Get stored paths
	var paths = load();
	// Add path for domain
