			...
			var sessionRequest = Request({
				...
				onComplete: function (response) {
					if (response.json) {
						...
						// Notify callback
						callback(response.json.success);
					} else {
...
connectionPanel.setStep = function (currentStep, lastStep) {
	connectionPanel.port.emit("set-step", currentStep + " / " + currentStep);
};
...
var sessionCallback = function (sessionOpened) {
	...
	if (sessionOpened) {
		...
		// Update connection status
		connectionPanel.setStatus("Merci de vérifier l'autorisation de l'extension.", "error");
	}
}
