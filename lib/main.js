...
var freebox = {
	...
	login: {
		...
		login: function (callback) {
			...
			Request({
				anon: true,
				url: "http://mafreebox.freebox.fr/api/v1/login/",
				onComplete: function (response) {
					// Check response json data
					if (response.json && response.json.success === true) {
						// Return challenge
	...
	fs: {
		...
		mkdir: function (parent, dirname) {
			...
			Request({
				anon: true,
				url: "http://mafreebox.freebox.fr/api/v1/fs/mkdir/",
				headers: {
					"X-Fbx-App-Auth": session_token
				},
				content: content,
...
var windowTrackerDelegate = {
	...
	onTrack: function (window) {
		...
		// Get default download path and location
		var downloadDomain = LocationManager.getDomain(downloadUrl);
		var defaultDownloadPath = LocationManager.getByDomain(downloadDomain);
		var downloadLocation = "";
		// Check if default download path is defined
		if (defaultDownloadPath === null) {
			// Set default download path
			downloadPath = LocationManager.getDefault();
		} else {
			// Set path from history
			downloadPath = defaultDownloadPath.path;
			// Check always use path
			if (defaultDownloadPath.always) {
				// Set download location
				downloadLocation = LocationManager.decodePath(defaultDownloadPath.path);
				if (downloadLocation === "")
					downloadLocation = " à la racine";
				else
					downloadLocation = " dans " + downloadLocation;
			}
		}
		/*
		 * Edit XUL window.
		...
		freeboxElement.setAttribute("label", "Envoyer sur la Freebox" + downloadLocation);
		// Create hbox element
		...
		window.dialog.onOK = function () {
			...
			if (freeboxElement.selected) {
				...
				if (downloadLocation !== "") {
					...
					LocationManager.setLast(downloadPath);
				...
				Timers.setTimeout(function () {
					browserPanel.open(downloadPath, function(path, remember) {
						...
						LocationManager.save(downloadDomain, downloadPath, remember);
						// Save last path to the manager
						LocationManager.setLast(downloadPath);
						// Add the download
						freebox.downloads.add(downloadUrl, downloadPath);
					});
				}, 500);
				// Validate dialog
				return true;
...
// Create connection panel API
browserPanel.open = function (path, callback) {
	// Store callback
...
browserPanel.lsRecursiveCallback = function (path, content, stack) {
	...
	if (content[0].path === content[1].path) {
		...
		for (var index = stack.length-1; index>=0; index--) {
			...
			if (index==0) {
				// Show last past
				browserPanel.port.emit('show', result.path);
			}
		}
		// Display panel
		browserPanel.show();
...
browserPanel.port.on("select", function (path, remember) {
	...
	if (typeof browserPanel.callback === "function") {
		// Call the callback
		browserPanel.callback(path, remember);
		// Remove the callback
...
locationsPanel.port.on("edit", function (domain) {
	// Get the path for the domain
	var path = LocationManager.getAll()[domain].path;
...
Tabs.on("ready", function (tab) {
	...
	// Send current download path
	worker.port.emit("pwd", downloadPathName);
	// Send icon resources
	...
	worker.port.on("browse", function () {
		...
		browserPanel.open(downloadPath, function() {
			...
			for (var index in magnetTabWorkers) {
				...
				// Notify worker of current download path
				magnetTabWorker.port.emit("pwd", downloadPathName);
			}
		});
	});
	worker.port.on("download", function (downloadUrl) {
		...
		freebox.downloads.add(downloadUrl, downloadPath, false);
	});
});
