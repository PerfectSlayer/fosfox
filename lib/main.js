...
var windowTrackerDelegate = {
	...
	onTrack: function (window) {
		...
		// Check if download path is defined
		if (defaultDownloadPath === null) {
			defaultDownloadPath = LocationManager.getDefault();
		} else {
			downloadLocation = LocationManager.decodePath(defaultDownloadPath);
			if (downloadLocation === "")
			...
			if (freeboxElement.selected) {
				...
				Timers.setTimeout(function () {
					browserPanel.open(downloadPath, function(path, remember) {
						...
						if (remember)
							LocationManager.save(downloadDomain, downloadPath);
						// Save last path to the manager
						LocationManager.setLast(downloadPath);
						// Add the download
...
locationsPanel.open = function () {
	...
	for (var domain in paths) {
		locations[domain] = LocationManager.decodePath(paths[domain]);
	}
