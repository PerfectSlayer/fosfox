		...
		add: function (url, path) {
			...
			Request({
				...
				onComplete: function (response) {
					...
					} else if (response.status === 403 && response.json.result.challenge) {
						// Try to reconnect application
						reconnectFallback(response.json.result.challenge, function () {
 ...
 */
var reconnectFallback = function (challenge, call) {
	// Log application with the given challenge
	freebox.login.session(challenge, function (sessionOpened, errorCode) {
